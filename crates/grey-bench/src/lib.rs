...
pub fn grey_sort_blob(n: u32) -> Vec<u8> {
    ...
    fn load_imm_64(c: &mut Vec<u8>, m: &mut Vec<u8>, rd: u8, val: u64) {
        c.push(20); m.push(1); c.push(rd); m.push(0);
        for i in 0..8 { c.push((val >> (i*8)) as u8); m.push(0); }
    }
    fn add_imm_64(c: &mut Vec<u8>, m: &mut Vec<u8>, rd: u8, ra: u8, imm: i32) {
        c.push(149); m.push(1); c.push(rd | (ra << 4)); m.push(0);
        for b in imm.to_le_bytes() { c.push(b); m.push(0); }
    }
    fn mov(c: &mut Vec<u8>, m: &mut Vec<u8>, rd: u8, ra: u8) {
        c.push(100); m.push(1); c.push(rd | (ra << 4)); m.push(0);
    }
    fn sub_64(c: &mut Vec<u8>, m: &mut Vec<u8>, rd: u8, ra: u8, rb: u8) {
        c.push(201); m.push(1); c.push(ra | (rb << 4)); m.push(0); c.push(rd); m.push(0);
    }
    fn add_64(c: &mut Vec<u8>, m: &mut Vec<u8>, rd: u8, ra: u8, rb: u8) {
        c.push(200); m.push(1); c.push(ra | (rb << 4)); m.push(0); c.push(rd); m.push(0);
    }
    fn store_ind_u32(c: &mut Vec<u8>, m: &mut Vec<u8>, rd: u8, ra: u8, imm: i32) {
        c.push(122); m.push(1); c.push(rd | (ra << 4)); m.push(0);
        for b in imm.to_le_bytes() { c.push(b); m.push(0); }
    }
    fn load_ind_u32(c: &mut Vec<u8>, m: &mut Vec<u8>, rd: u8, ra: u8, imm: i32) {
        c.push(128); m.push(1); c.push(rd | (ra << 4)); m.push(0);
        for b in imm.to_le_bytes() { c.push(b); m.push(0); }
    }
    ...
    let cmp_pc = c.len();
    branch_lt_u(&mut c, &mut m, T2, A0, (inner_loop_pc as i32) - (cmp_pc as i32));
    ...
    add_imm_64(&mut c, &mut m, T0, T0, 1);     // i++
    let outer_br = c.len();
    branch_lt_u(&mut c, &mut m, T0, S1, (outer_loop_pc as i32) - (outer_br as i32));
