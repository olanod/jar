...
//!
//! Compiled to RISC-V (riscv32em), then transpiled to PVM bytecode for
//! both grey and polkavm benchmarks. Uses the k256 crate (pure Rust).
...
const EXPECTED_PUBKEY: [u8; 33] = [
    0x02, 0x84, 0xbf, 0x75, 0x62, 0x26, 0x2b, 0xbd,
    0x69, 0x40, 0x08, 0x57, 0x48, 0xf3, 0xbe, 0x6a,
    0xfa, 0x52, 0xae, 0x31, 0x71, 0x55, 0x18, 0x1e,
    0xce, 0x31, 0xb6, 0x63, 0x51, 0xcc, 0xff, 0xa4,
    0xb0,
];
...
pub extern "C" fn ecrecover_bench() -> u32 {
    ...
    match k256::ecdsa::VerifyingKey::recover_from_prehash(&MSG_HASH, &sig, recid) {
        Ok(key) => {
            ...
            for i in 0..EXPECTED_PUBKEY.len() {
